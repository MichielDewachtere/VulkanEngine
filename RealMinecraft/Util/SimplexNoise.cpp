 ...
 */
static float grad(int32_t hash, double x) {
    ...
    return static_cast<float>(grad * x);              // Multiply the gradient with the distance
}
 ...
 * @return gradient value
 */
static float grad(int32_t hash, double x, double y) {
    const int32_t h = hash & 0x3F;  // Convert low 3 bits of hash code
    const double u = h < 4 ? x : y;  // into 8 simple gradient directions,
    const double v = h < 4 ? y : x;
    return static_cast<float>((h & 1) ? -u : u) + static_cast<float>((h & 2) ? -2.0 * v : 2.0 * v); // and compute the dot product with (x,y).
}
 ...
 * @return gradient value
 */
static float grad(int32_t hash, double x, double y, double z) {
    ...
    return static_cast<float>((h & 1) ? -u : u) + static_cast<float>((h & 2) ? -v : v);
}
 ...
 */
float SimplexNoise::noise(double x) {
   ...
   double x1 = x0 - 1.0;
    ...
    return 0.395f * static_cast<float>(n0 + n1);
}
...
float SimplexNoise::noise(double x, double y) {
    ...
    static constexpr double F2 = 0.366025403;  // F2 = (sqrt(3) - 1) / 2
    static constexpr double G2 = 0.211324865;  // G2 = (3 - sqrt(3)) / 6   = F2 / (1 + 2 * K)
    ...
    return 45.23065f * static_cast<float>(n0 + n1 + n2);
}
 ...
 * @return Noise value in the range[-1; 1], value of 0 on all integer coordinates.
 */
float SimplexNoise::noise(double x, double y, double z) {
    double n0, n1, n2, n3; // Noise contributions from the four corners

    // Skewing/Unskewing factors for 3D
    static const double F3 = 0.333333333;
    static const double G3 = 0.166666666;
    ...
    return 32.0f * static_cast<float>(n0 + n1 + n2 + n3);
}
 ...
 * @return Noise value in the range[-1; 1], value of 0 on all integer coordinates.
 */
float SimplexNoise::fractal(size_t octaves, double x) const {
    float output = 0.f;
    float denom = 0.f;
    ...
    for (size_t i = 0; i < octaves; i++) {
        ...
        frequency *= static_cast<double>(mLacunarity);
 ...
 * @return Noise value in the range[-1; 1], value of 0 on all integer coordinates.
 */
float SimplexNoise::fractal(size_t octaves, double x, double y) const {
    float output = 0.f;
    float denom = 0.f;
    double frequency = mFrequency;
    float amplitude = mAmplitude;
 ...
 * @return Noise value in the range[-1; 1], value of 0 on all integer coordinates.
 */
float SimplexNoise::fractal(size_t octaves, double x, double y, double z) const {
    float output = 0.f;
    float denom = 0.f;
    double frequency = mFrequency;
    float amplitude = mAmplitude;
