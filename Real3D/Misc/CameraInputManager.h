...
class CameraInputManager final : public real::Singleton<CameraInputManager>
{
public:
	~CameraInputManager() override = default;

	CameraInputManager(const CameraInputManager&) = delete;
	CameraInputManager& operator=(const CameraInputManager&) = delete;
	CameraInputManager(CameraInputManager&&) = delete;
	CameraInputManager& operator=(CameraInputManager&&) = delete;

	void Initialize(const real::GameContext& context);
	void CleanUp() const;

	void UpdateInputStates(const real::GameContext& context, bool override);
...
private:
	friend class Singleton<CameraInputManager>;
	CameraInputManager() = default;
	...
	void UpdateMouseStates(const real::GameContext& context);
