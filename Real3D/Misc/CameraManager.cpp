...
void real::CameraManager::HandleEvent(real::SceneEvents events, real::Scene*)
{
	if (events == real::SceneEvents::exit)
		m_pCameras.clear();
}

uint32_t real::CameraManager::AddCamera(Camera* camera, bool setActive)
{
	if (camera == nullptr)
	{
...
void real::CameraManager::SetCameraActive(Camera* camera)
{
	AddCamera(camera);
...
void real::CameraManager::SetCameraActive(uint32_t camera)
{
...
real::Camera* real::CameraManager::GetActiveCamera()
{
